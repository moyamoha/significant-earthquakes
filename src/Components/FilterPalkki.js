...
const FilterPalkki = ({ setQuake, setChanged, filterObj, setFilterObj }) => {
    ...
    return (
        <div className="col-2 listapalkki ">
            <div id="opt" >
                <label className="h5 container p-0" >{t('year')}:</label>
                <div><small>{t('from')}...</small></div>
                <input id="minYearLimit" type="number" placeholder={t('year')} min="0" max="2021" onChange={(e) => changeFilterObj(e.target.value, smaxyear, loc, sminMagn, smaxMagn, sminDeaths, smaxDeaths)} />
                <div><small>{t('to')}..</small></div>
                <input id="maxYearLimit" type="number" placeholder={t('year')} min="0" max="2021" onChange={(e) => { changeFilterObj(sminyear, e.target.value, loc, sminMagn, smaxMagn, sminDeaths, smaxDeaths) }} />
            </div>
            <div >
                <label className="h5 container p-0 pt-3 ">{t('location')}:</label>
                <input list="loclist" id="searchTerm" type="text" placeholder={t('loc_name')} onChange={(e) => { changeFilterObj(sminyear, smaxyear, e.target.value, sminMagn, smaxMagn, sminDeaths, smaxDeaths) }} />
                <datalist id="loclist">
            ...
            <div>
                <label className="h5 container p-0 pt-3">{t('magnitude')}:</label>
                <div><small>{t('from')}...</small></div>
            ...
            <div>
                <label className="h5 container p-0 pt-3">{t('deaths')}:</label>
                <div><small>{t('from')}...</small></div>
